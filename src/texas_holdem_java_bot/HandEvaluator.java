...
public class HandEvaluator {
    ...
    protected void sortByRank(){
        Arrays.sort(availableCards, new ValueComparator());
    }
    ...
    protected String evaluateHand(){
        ...
        for(int i=0; i<availableCards.length;i++){
            rankCounter[ availableCards[i].getValueValue() ]++;
            suitCounter[ availableCards[i].getSuitValue() ]++;
        }
        ...
        if ((rankCounter[9] >= 1 &&       /* 10 */
            ...
            royalSearch:
                for (int i=0;i<3;i++){
                    ...
                    if (availableCards[i].getValueValue() == 0){
                        ...
                        for (int j=1;j<4-i;j++){
                            if ((availableCards[i+j].getValueValue() == 9 && 
                                    availableCards[i+j+1].getValueValue() == 10 &&
                                    availableCards[i+j+2].getValueValue() == 11 &&
                                    availableCards[i+j+3].getValueValue() == 12) 
                                    &&
    ...
    private String evaluateStraightFlush(short[] rankCounter, short[] suitCounter){
        ...
        if (suitCounter[0] > 4 || suitCounter[1] > 4 ||
            ...
            for (int i=availableCards.length-1;i>3;i--){
                if ((availableCards[i].getValueValue()-ONE == availableCards[i-ONE].getValueValue() && 
                        availableCards[i].getValueValue()-TWO == availableCards[i-TWO].getValueValue() &&
                        availableCards[i].getValueValue()-THREE == availableCards[i-THREE].getValueValue() &&
                        availableCards[i].getValueValue()-FOUR == availableCards[i-FOUR].getValueValue()) 
                        &&
                        ...
                        availableCards[i].getSuit() == availableCards[i-FOUR].getSuit())){
                            // Found royal flush, break and return.
                            result = "Straight Flush!! ";// + Card.rankAsString(availableCards[i].getValueValue()) + " high of " + Card.suitAsString(availableCards[i].getSuit());
                            break;
                }
            }
        }
        ...
        if (suitCounter[0] > 4 || suitCounter[1] > 4 ||
            ...
            for (int i=availableCards.length-1;i>3;i--){
                if (availableCards[i].getSuit() == availableCards[i-ONE].getSuit() &&
                        ...
                        availableCards[i].getSuit() == availableCards[i-FOUR].getSuit()){
                            // Found royal flush, break and return.
                            result = "Flush!! ";// + Card.rankAsString(availableCards[i].getValueValue()) + " high of " + Card.suitAsString(availableCards[i].getSuit());
                            break;
                }
            }           
        }
